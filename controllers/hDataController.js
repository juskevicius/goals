...
exports.hData_update_post = (req, res, next) => {
  ...
  function calcHistoricalData(theGoal) {
    theGoal.parentTo = theGoal.parentTo.filter(child => child.status === 'Approved');
    const dates = theGoal.parentTo.length > 0 && theGoal.parentTo.map(child => child.history.data.map(entry => entry.date.getTime())); /* extract all dates from child goals */
    const datesArr = dates.length > 0 && dates.reduce((arr, val) => arr.concat(val)).sort((a, b) => (a - b)); /* build an array and sort it in ascending order */
    const uniqueDates = datesArr.length > 0 && datesArr.filter((v, i) => datesArr.indexOf(v) === i); /* remove duplicates */
    const childData = theGoal.parentTo.length > 0 && theGoal.parentTo.map(child => child.history.data.map(entry => [entry.date.getTime(), entry.value, child.weight])); /* build ar array of arrays for children data [date, value, weight] */
    let childArr = childData.length > 0 && childData.map(unit => unit.sort((a, b) => (a[0] - b[0]))); /* sort children dates in ascending order  */
    let currChildScore = [];
    let calcHistData = [];
    for (let i = 0; i < uniqueDates.length; i += 1) { /* loop through unique dates */
      let currWeight = 0;
      let sum = 0;
      let weight = 0;
      for (let j = 0; j < childArr.length; j += 1) { /* loop through children */
        ...
        sum = currChildScore[j] * currWeight + sum; /* multiply the current score by it's weight and add to the sum */
        weight = currWeight + weight; /* calculate the sum of weights */
      }
  ...
  Goal
    ...
    .exec((err, parentGoal) => {
      ...
      } else if (res.locals.updateTasks) {
        next();
      } else {
...
exports.tasks_update_post =  (req, res, next) => {
  function calcTaskImpl(theGoal) {
    for (let t = 0; t < theGoal.task.length; t += 1) {
      const currTask = theGoal.task[t].description;
      let impl = [];
      let weight = [];
      let sumWeight = 0;
      let sum = 0;
      for (let i = 0; i < theGoal.parentTo.length; i += 1) { /* loop through goals of children */
        for (let j = 0; j < theGoal.parentTo[i].task.length; j += 1) { /* loo through tasks in children goals */
          if (currTask === theGoal.parentTo[i].task[j].description && theGoal.parentTo[i].status === 'Approved') { /* if current task is found in a child's goal  */
            ...
            sumWeight = (theGoal.parentTo[i].weight ? theGoal.parentTo[i].weight : 100) + sumWeight; /* a sum of all weights. Required to be able to calculate the weighted average */
          }
        }
      }
      for (let i = 0; i < impl.length; i += 1) {
        sum = impl[i] * weight[i] + sum; /* multiply weights and implementation statuses */
      }
