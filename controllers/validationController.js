...
exports.user_login_post = [
  body('empId').trim().escape().isLength({ min: 1 }).trim().withMessage('EmpId empty.'),
  body('password').trim().escape().isLength({ min: 1 }).trim().withMessage('Password empty.'),
...
exports.goal_add_post = [
  body('name').trim().escape().isLength({ min: 1, max: 100 }).trim().withMessage('Goal failure. Max length is 100 sybmols.'),
  ...
  body('comment').trim().escape().isLength({ max: 400 }).trim().withMessage('Comment is too long'),
...
exports.goal_details_get = [
  param('id').trim().escape().custom(id => {
...
exports.goal_score_post = [
  ...
  body('entryId').trim().escape().optional({ checkFalsy: true }).custom(id => {
    return id.match(/^[a-fA-F0-9]{24}$/) ? true : false;
  }).withMessage('id is not valid'),
...
exports.goal_taskImplementation_post = [
  ...
  body('taskId').trim().escape().custom(id => {
    return id.match(/^[a-fA-F0-9]{24}$/) ? true : false;
